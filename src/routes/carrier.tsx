...
function CarrierPortal() {
  const [carrierName, setCarrierName] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("carrierName") || "";
    }
    return "";
  });
  ...
  }, [carrierName, refreshLoads]);

  const availableLoads = loads.filter((l) => l.status === "posted");
  const myTrips = loads.filter((l) => l.carrierName === carrierName);
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {!carrierName.trim() ? (
        <div className="mx-auto max-w-md rounded-2xl border border-gray-200 bg-white p-8 dark:border-gray-700 dark:bg-gray-900">
          ...
          <input
            type="text"
            placeholder="e.g. Cross-Border Hauliers Ltd"
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && carrierName.trim()) {
                localStorage.setItem("carrierName", carrierName.trim());
                setLoading(true);
              }
            }}
          />
